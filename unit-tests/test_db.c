...
static void test_db(void **state)
{
    ...
    strncpy(program_id, "credits.aleo", sizeof(program_id));
    strncpy(function_name, "bond_public", sizeof(function_name));
    program_length       = strlen(program_id);
    function_name_length = strlen(function_name);
    status               = db_get_function_parameters(
        program_id, program_length, function_name, function_name_length, &function_parameters);
    assert_int_equal(status, 0);
    assert_int_equal(function_parameters->tx_type, TX_STAKING);
    assert_int_equal(function_parameters->input_count, 3);

    strncpy(program_id, "credits.aleo", sizeof(program_id));
    strncpy(function_name, "unbond_public", sizeof(function_name));
    program_length       = strlen(program_id);
    function_name_length = strlen(function_name);
    status               = db_get_function_parameters(
        program_id, program_length, function_name, function_name_length, &function_parameters);
    assert_int_equal(status, 0);
    assert_int_equal(function_parameters->tx_type, TX_STAKING);
    assert_int_equal(function_parameters->input_count, 2);

    strncpy(program_id, "credits.aleo", sizeof(program_id));
    strncpy(function_name, "claim_unbond_public", sizeof(function_name));
    program_length       = strlen(program_id);
    function_name_length = strlen(function_name);
    status               = db_get_function_parameters(
        program_id, program_length, function_name, function_name_length, &function_parameters);
    assert_int_equal(status, 0);
    assert_int_equal(function_parameters->tx_type, TX_STAKING);
    assert_int_equal(function_parameters->input_count, 1);
