...
static void cx_twisted_edwards_dbl(cx_mpi_ecpoint_t *p, cx_mpi_t *p_t, cx_mpi_t *mpi[])
{
    mpi[X1] = p->x;
    mpi[Y1] = p->y;
    if (p_t != NULL) {
...
static void cx_twisted_edwards_add_simple(cx_mpi_ecpoint_t *p, cx_mpi_ecpoint_t *q, cx_mpi_t *mpi[])
{
    mpi[X1] = p->x;
    mpi[Y1] = p->y;
    mpi[Z1] = p->z;
...
static void cx_twisted_edwards_add(cx_mpi_ecpoint_t *p,
                                   cx_mpi_t         *p_t,
                                   cx_mpi_ecpoint_t *q,
                                   cx_mpi_t         *q_t,
                                   cx_mpi_t         *mpi[])
...
static uint32_t cx_internal_get_bit(const uint8_t *k, uint32_t k_len, uint32_t pos)
{
...
cx_err_t cx_twisted_edwards_recover_x(cx_mpi_ecpoint_t *P, uint32_t sign)
{
    ...
    if (((p = cx_mpi_alloc_init(&bn_p, sz, domain->p, sz)) == NULL)
        || ((t1 = cx_mpi_alloc(&bn_t1, sz)) == NULL) || ((t2 = cx_mpi_alloc(&bn_t2, sz)) == NULL)
...
cx_err_t cx_twisted_edwards_mul_point(cx_mpi_ecpoint_t *P, const uint8_t *k, uint32_t k_len)
{
    cx_err_t                          error;
    cx_mpi_t                         *mpi[MAX_ID];
    cx_bn_t                           bn[MAX_ID];
