...
static int cx_weierstrass_mult(cx_curve_t curve,
                               cx_mpi_t  *qx,
                               cx_mpi_t  *qy,
                               cx_mpi_t  *px,
                               cx_mpi_t  *py,
                                       ...
                                       bool                set_coord)
{
    ...
    if (p != NULL) {
        ...
        if (cx_mpi_ecpoint_from_ecpoint(&bP, ec_P) != CX_OK
            || (set_coord && EC_POINT_set_affine_coordinates(group, p, bP.x, bP.y, ctx) != 1)) {
...
static cx_err_t cx_ecpoint_normalize(cx_mpi_ecpoint_t *P, const cx_ecpoint_t *ec_P)
{
    cx_err_t error;
...
cx_err_t sys_cx_ecpoint_export(const cx_ecpoint_t *ec_P,
                               uint8_t            *x,
                               size_t              x_len,
                               uint8_t            *y,
                               size_t              y_len)
                                            ...
                                            const cx_bn_t bn_r)
{
    ...
    if (BN_bn2binpad(k, kbytes, k_len) != -1 && BN_bn2binpad(r, rbytes, r_len) != -1) {
        error = sys_cx_ecpoint_double_scalarmul(ec_R, ec_P, ec_Q, kbytes, k_len, rbytes, r_len);
    }
    else {
        error = CX_INTERNAL_ERROR;
    }
...
cx_err_t sys_cx_ecpoint_add(cx_ecpoint_t *ec_R, const cx_ecpoint_t *ec_P, const cx_ecpoint_t *ec_Q)
{
    ...
    else if ((ec_P->curve == CX_CURVE_EdBLS12) || (ec_P->curve == CX_CURVE_JUBJUB)) {
...
cx_err_t sys_cx_ecpoint_cmp(const cx_ecpoint_t *ec_P, const cx_ecpoint_t *ec_Q, bool *is_equal)
{
    ...
    *is_equal = (P.curve == Q.curve) && (BN_cmp(P.x, Q.x) == 0) && (BN_cmp(P.y, Q.y) == 0);
end:
    return error;
}
...
cx_err_t sys_cx_ecpoint_neg(cx_ecpoint_t *ec_P)
{
    ...
    if ((n = cx_mpi_alloc_init(&bn_n, domain->length, domain->p, domain->length)) == NULL) {
        return CX_MEMORY_FULL;
    }
    coord = NULL;
...
cx_err_t sys_cx_ecpoint_x25519(const cx_bn_t bn_u, const uint8_t *k, size_t k_len)
{
    uint8_t  *u_bytes;
    int       nbytes;
    cx_mpi_t *u;
