...
cx_err_t cx_montgomery_recover_y(cx_mpi_ecpoint_t *P, uint32_t sign)
{
    ...
    if (((p = cx_mpi_alloc_init(&bn_p, sz, domain->p, sz)) == NULL)
        || ((sqy = cx_mpi_alloc(&bn_sqy, sz)) == NULL) || ((t1 = cx_mpi_alloc(&bn_t1, sz)) == NULL)
        || ((t2 = cx_mpi_alloc(&bn_t2, sz)) == NULL) || ((t3 = cx_mpi_alloc(&bn_t3, sz)) == NULL)) {
...
static void cx_decode_scalar(cx_curve_t curve, const uint8_t *k, uint8_t *k_enc, size_t k_len)
{
    uint8_t buf[MAX_MONT_BYTE_LEN];
 ...
 * @return Error code
 */
cx_err_t cx_montgomery_mul_coordinate(cx_curve_t     curve,
                                      cx_mpi_t      *u_coordinate,
                                      ...
                                      size_t         scalar_len)
{
    ...
    if ((domain == NULL) || ((curve != CX_CURVE_Curve448) && (curve != CX_CURVE_Curve25519))) {
        return CX_INVALID_PARAMETER;
    }
