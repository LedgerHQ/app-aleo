                           ...
                           cx_mpi_t    **m)
{
    ...
    if (bn_a < MAX_MPI_ARRAY_ENTRIES && bn_b < MAX_MPI_ARRAY_ENTRIES && bn_r < MAX_MPI_ARRAY_ENTRIES
        && bn_m < MAX_MPI_ARRAY_ENTRIES) {
        *a = cx_mpi_array[bn_a].mpi;
        *b = cx_mpi_array[bn_b].mpi;
        *r = cx_mpi_array[bn_r].mpi;
...
int cx_mpi_cmp(cx_mpi_t *a, cx_mpi_t *b)
{
    ...
    if (a_len == b_len) {
        ...
        if (BN_bn2binpad(a, a_ptr, a_len) != -1 && BN_bn2binpad(b, b_ptr, b_len) != -1) {
...
{
    cx_err_t error;
    uint8_t  a_ptr[MAX_MPI_BYTE_SIZE], b_ptr[MAX_MPI_BYTE_SIZE], r_ptr[MAX_MPI_BYTE_SIZE];
...
cx_err_t cx_mpi_gf2_n_mul(cx_mpi_t       *r,
                          const cx_mpi_t *a,
                          const cx_mpi_t *b,
                          const cx_mpi_t *n,
