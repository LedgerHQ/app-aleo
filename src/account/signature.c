...
static int hash_external_record_input(prepared_request_t *request, uint8_t input_index)
{
    ...
    if (num_fields > (HASH_INPUT_MAX_LENGTH - 11)) {  /* 11 = 8 capacity + function_id + tvk + index */
        return -1;
    }

    _Static_assert(HASH_INPUT_MAX_LENGTH >= 12, "hash_input size won't fit for external record");
    memset(hash_input, 0, sizeof(hash_input));
