...
void swap_handle_get_printable_amount(get_printable_amount_parameters_t *params)
{
    ...
    uint8_t decimals                = ALEO_DECIMALS;
    char    ticker[MAX_TICKER_SIZE] = ALEO_TICKER;
    if (params->is_fee || params->coin_configuration == NULL) {
        PRINTF("Defaulting to native ALEO amount\n");
    }
    else {
    ...
    char formatted_amount[MAX_AMOUNT_SIZE + 1] = {0};
    if (!format_fpu128(formatted_amount, sizeof(formatted_amount), raw_amount, decimals)) {
        PRINTF("Fail to format amount\n");
        return;
    }
