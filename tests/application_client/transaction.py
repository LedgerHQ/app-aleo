...
class Transaction:
    ...
    def generate_input(inputs: dict):
        ...
        for input_item in inputs:
            val += Transaction.forge_tlv(
                Transaction.TlvTypes.INPUT_TYPES,
                Transaction.get_input_type_from_string(input_item["type"]),
            )
            input_val = ""
    ...
    def generate_request(request: dict, is_root: bool) -> str:
        ...
        if is_root:
            val += Transaction.forge_tlv(
                Transaction.TlvTypes.NESTED_CALL_COUNT,
                f"{request['nested_call_count']:02x}",
            )
    ...
    def gen_intent_apdu(self, tx: dict) -> list[str]:
        ...
        req += Transaction.forge_tlv(
            ...
            Transaction.generate_request(tx["request"], True),
        )
    ...
    def unpack_response(self, response: str):
        ...
        while offset < len(response):
            ...
            if t == Transaction.TlvTypes.STRUCTURE_TYPE:
                ...
                unpacked["signature"] = {"pk_sig": v[128:192], "pr_sig": v[192:256]}
