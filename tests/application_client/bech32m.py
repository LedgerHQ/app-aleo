...
class BECH32M():
    ...
    @staticmethod
    def convert_bits(output: list[int], output_bits: int, input_data: list[int], input_bits: int, pad: bool) -> bool:
        val = 0
        bits = 0
        max_v = (1 << output_bits) - 1
        for item in input_data:
            val = (val << input_bits) | item
            bits += input_bits
    ...
    @staticmethod
    def decode(hrp: list[int], data: list[int], input_data: list[int]) -> bool:
        chk = 1
        if len(input_data) < 8 or len(input_data) > 90:
            return False
