...
```text
swap/
|── cal_helper.py                     # A fake CAL configuration for the ALEO currency
├── conftest.py                       # Pytest fixtures and device setup
├── test_aleo.py                      # Functional test cases
...
```sh
docker run --user "$(id -u)":"$(id -g)" --rm -ti -v "$(realpath .):/app" "ghcr.io/ledgerhq/ledger-app-builder/ledger-app-dev-tools:latest" python3 helper_tool_build_dependencies.py
```
